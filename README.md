...
- [架构说明](docs/ARCHITECTURE.md)：主题、启动器、注入流程、双布局与桌宠的整体关系。
- [效果预览](#效果预览)：查看两种背景构图的实际截图。
- [一键安装](#一键安装)：安装主题与配套桌宠。
...
启动时会按所选构图生成对应主题，然后完成一次注入，脚本随即退出。Codex 独立于这个终端运行；关闭终端不会影响 Codex，也不会启动监听器。需要关闭 Codex 时请在 App 内使用 `⌘Q` 正常退出。
